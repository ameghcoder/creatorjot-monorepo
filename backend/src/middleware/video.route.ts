...
import { PayloadSchema } from "../modules/video/video.schema.js";
import { savePayloads } from "../modules/db/db.payloads.js";
import { supabase } from "../lib/supabase.js";
import { extractVideoId } from "../services/youtube.service.js";
import { getOrFetchTranscript } from "../modules/db/db.transcript.js";
import {
  getUserActivePlan,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const headings = richContext.post_angles
        .map((angle, index) => ({ post_angle_index: index, score: angle.score, hook: angle.hook }))
    ...
    try {
      ...
      if (parsed.data.sessionId) {
        ...
        if (existingSession.status !== "active") {
          logger.error("Current session is not active", { sessionId: existingSession.id, status: existingSession.status });
          res.status(403).json({
            error: "SessionInactive",
            message: "Your current session is not active. Please start a new session or activate the current one.",
            status: 403,
          });
          return;
        }
      ...
      if (autoGenerate && platforms.length > 0) {
        ...
        if (userPlan.plan_type === "free") {
          ...
          validateFreeUserRequest(userPlan, platforms[0], xPostFormat, durationMinutes);
        } else {
          ...
          for (const platform of platforms) {
            ...
            totalGenerationCost += computeTotalCost(userPlan, platform, fmt, 0, true);
          }
          const videoCost = isVideoCached ? 0 : calculateVideoProcessingCost(durationMinutes);
          validateProUserRequest(userPlan, videoCost + totalGenerationCost, durationMinutes);
        }
      } else if (!autoGenerate) {
        // No generation requested — still enforce duration limits
        if (userPlan.plan_type === "free" && durationMinutes > 10) {
          throw new AppError("Videos longer than 10 minutes require a Pro plan.", 400);
        } else if (userPlan.plan_type === "pro" && durationMinutes > 90) {
          throw new AppError("Videos longer than 90 minutes are not supported.", 400);
        }
      }

      // Log video_processing event (free users: 0 credits; pro: cost if not cached)
      const videoProcessingCost = (userPlan.plan_type === "pro" && !isVideoCached)
      ...
      if (!hasAISummary) {
        ...
        const transcriptJobResult = await jobManagementService.enqueueTranscriptJob({
          userId: req.user!.id,
          ytId: extractedId,
          sessionId: sessionId,
          userTier: userTier,
        ...
        for (const platform of platforms) {
          const generationResult = await jobManagementService.enqueueGenerationJob({
            userId: req.user!.id,
            transcriptId: transcriptData.transcript_id,
            payloadId: payloadId,
            sessionId: sessionId,
      ...
      res.status(201).json({
        ...
        queue_info: {
          ...
          generation_deferred: transcriptJobId !== null && autoGenerate && platforms.length > 0,
        },
      });
    } catch (error) {
      next(error);
